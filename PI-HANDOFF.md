...
- **HF 오프라인**: `macos-tts-server.py:31-32` 가 `HF_HUB_OFFLINE/TRANSFORMERS_OFFLINE=1` 을 건다.
  → 캐시에 없는 HF 모델은 전부 로드 실패한다. 서버 기동 시 HF 에서 받으려는 모델은 **4종**:
  melo(`MeloTTS-Korean`, `kykim/bert-kor-base`), STT(`Systran/faster-whisper-small`),
  OpenVoice 워터마크(`wavmark.load_model()` — `api.py:107`). `fetch-models.sh` 가 이 4종을 미리 받는다.
  증상별 로그:
    - `⚠️ OpenVoiceV2 로드 실패 ... locate the file on the Hub` → wavmark 미캐시(음색변환 안 됨, melo 원본으로 폴백)
    - `⚠️ STT 워밍업 건너뜀 ... outgoing traffic has been disabled` → faster-whisper 미캐시(마이크 STT 안 됨)
  → `fetch-models.sh` 를 (다시) 돌리거나, **최초 1회만** 서버를 온라인으로 띄워 받는다(둘 다 캐시됨):
  `HF_HUB_OFFLINE=0 TRANSFORMERS_OFFLINE=0 python tts-server/macos-tts-server.py --backend melo --serve-dir virtual-avatar/dist --port 8080`
  받아진 뒤 다시 평소(오프라인)대로 실행하면 캐시에서 로드된다.
